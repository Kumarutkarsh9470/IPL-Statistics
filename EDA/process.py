...
if not DATA_PATH.exists():
    # Fallback for if it's still in the current directory (unlikely but safe)
    if Path("IPL.csv").exists():
        DATA_PATH = Path("IPL.csv")
    else:
        raise FileNotFoundError(f"Could not find IPL.csv at {DATA_PATH} or in current folder.")
...
# Save Cleaned CSV for Database Ingestion and Portal sync
OUTPUT_CSV_DIR = PROJECT_ROOT / "data" / "processed"
OUTPUT_CSV_DIR.mkdir(parents=True, exist_ok=True)
CLEANED_CSV_PATH = OUTPUT_CSV_DIR / "cleaned_IPL.csv"
df.to_csv(CLEANED_CSV_PATH, index=False)
print(f"Data Cleaning Complete. Saved permanent clean dataset to: {CLEANED_CSV_PATH}")
...
INSIGHTS_MD = OUTPUT_DIR / 'week1_insights.md'
with open(INSIGHTS_MD, 'w') as f:
    f.write("# IPL Data Analysis - Week 1 Insights\n\n")
    f.write("Here are 12 key insights derived from the data with proposed visualization types for the final dashboard:\n\n")
    for ins in insights:
        f.write(f"### {ins['id']}. {ins['category']} - {str(ins.get('insight', '')).split('.')[0]}\n")
        f.write(f"- **Description**: {ins['insight']}\n")
        f.write(f"- **Proposed Visualization**: {ins['visualization']}\n\n")

print(f"EDA completed successfully. Outputs saved to: {OUTPUT_DIR}/")